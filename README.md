...
| `SCRAPER_CITY` / `SCRAPER_MAX_RENT` / `SCRAPER_INTERVAL_SECONDS` / `SCRAPER_REFRESH_HOURS` / `SCRAPER_DELETION_PASSES` / `SCRAPER_MAX_AGE_DAYS` | no | scraper | Tune the scraper loop. Defaults: `München` / `2000` / `300` / `24` / `2` / `7`. Pagination depth is no longer fixed: the agent walks each source until the first listing on a page is older than `SCRAPER_MAX_AGE_DAYS` |
