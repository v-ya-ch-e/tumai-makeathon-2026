...
backend/app/wg_agent/
  ...
  evaluator.py                   Matcher v2 scorecard (ADR-028, see [MATCHER.md](./MATCHER.md)): `hard_filter` + ten components (price / size / wg_size / availability / commute / preferences / tenancy / upfront_cost / vibe / quality) + `compose` with split match-vs-quality blend
  market.py                      District-level price percentile against the global `ListingRow` pool; consumed by `evaluator.price_fit` for evidence strings
...
- `vibe_score(listing, requirements, *, nearby_places=None, soft_signal_keys=None) -> VibeJudgement` — Single env-driven (`WG_VIBE_MODEL`, default `gpt-5.4-nano`) Chat Completions JSON-object call used by [`evaluator.vibe_fit`](../backend/app/wg_agent/evaluator.py). The prompt is explicitly told **not** to judge price, size, WG size, or commute — it judges prose-only vibe and emits four side channels the rest of the engine reuses: `soft_signal_scores` (per-key 0..1 for §3.4 prefs like `lgbt_friendly`/`wg_gender`), `tenancy_label` (one of `open_ended`/`long_term`/`mid_term`/`short_term`/`unknown`, used by `tenancy_fit` when `available_to` is missing), `scam_severity` (drives a 0.30 hard cap on the vibe component when ≥ 0.7), and `red_flags`/`green_flags` (folded into match/mismatch reasons). Validated as a Pydantic `VibeJudgement`; the legacy `VibeScore` is kept as a type alias.
...
Matcher v2 (ADR-028, [MATCHER.md](./MATCHER.md)). Pipeline: `hard_filter` → optional vibe LLM call (when the user has §3.4 prefs, runs first to populate side channels) → ten deterministic components → `compose` (weighted mean of `live` excluding `quality` → `min(raw, *caps)` → `0.85·match + 0.15·quality` post-blend). Public functions:
...
- `compose(components, *, veto=None, extra_match_reasons=None, extra_mismatch_reasons=None) -> EvaluationResult` — Excludes `quality` from the weighted mean; takes `min(raw, *caps)` over caps from non-missing components only (defensive — no stale caps from a failed component); blends `final = 0.85·match + 0.15·quality`. When `match_reasons` would be empty for a middle-band listing, falls back to the top-2 live components' headline evidence (same for `mismatch_reasons` ↔ bottom-2). Names the binding cap in `score_reason` (`"capped at 0.45 by deal-breaker commute to Marienplatz"`).
- `evaluate(listing, profile, *, travel_times=None, nearby_places=None, market_context=None) -> EvaluationResult` — End-to-end facade; what `UserAgent.run_match_pass` calls. Calls `vibe_fit` first when the user has §3.4 LLM prefs so its `soft_signal_scores` can feed `preference_fit` and its `tenancy_label` can feed `tenancy_fit`.

Curve tuning (weights, multipliers, distance bands) lives at the top of the module in `COMPONENT_WEIGHTS`, the named-multiplier block (`PRICE_VETO_MULT`, `COMMUTE_VETO_MULT`, `COMMUTE_CAP_MULT`, `COMMUTE_HARD_CAP`, `PREF_HARD_CAP_WEIGHT5` and friends, `SCAM_VIBE_HARD_CAP`, `QUALITY_BLEND_WEIGHT`, `DEFAULT_COMMUTE_BUDGET_MIN`), `STRUCTURED_PREFERENCES`, `KEYWORD_PREFERENCES`, and `LLM_PREFERENCES`. The pinned `(comfort, ok, max)` distance triples per Places category live in [`places.PLACE_DISTANCE_BANDS`](../backend/app/wg_agent/places.py) so the proof tests, production code, and docs cannot drift. Unit tests pin every curve's boundaries in [`test_evaluator.py`](../backend/tests/test_evaluator.py); preference-family routing is exercised in [`test_evaluator_resolvers.py`](../backend/tests/test_evaluator_resolvers.py); end-to-end behaviour on ten realistic listing/profile combinations is in [`test_evaluator_integration.py`](../backend/tests/test_evaluator_integration.py).
...
| [`test_evaluator.py`](../backend/tests/test_evaluator.py) | Matcher v2 evaluator: `hard_filter` paths (incl. `non_smoking` inversion + scored-anchor commute rule), every per-component curve, threshold-matrix multipliers, `compose` arithmetic with `quality` excluded from `live` and `0.85·match + 0.15·quality` blend, missing-cap defensiveness, `vibe_fit` graceful degradation, `upfront_cost_fit` boundaries (no LLM, no DB) | `cd backend && pytest tests/test_evaluator.py` |
| [`test_evaluator_resolvers.py`](../backend/tests/test_evaluator_resolvers.py) | Per-family preference resolvers (structured booleans incl. inversion, Places distance bands per category, regex with word boundaries that excludes `Bahnhof`/`unruhig`, LLM `soft_signal_scores` passthrough) (no LLM, no DB) | `cd backend && pytest tests/test_evaluator_resolvers.py` |
| [`test_evaluator_integration.py`](../backend/tests/test_evaluator_integration.py) | Ten realistic listing/profile combinations end-to-end with `brain.vibe_score` mocked (perfect, far-commute deal-breaker, missing must-have, sublet vs long-term, kalt_uplift, suspiciously cheap, district vetoed, no-photos, scam-flagged, all-data-missing) (no LLM, no DB) | `cd backend && pytest tests/test_evaluator_integration.py` |
