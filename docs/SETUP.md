...
2. **Environment file**
   ...
   - Optionally tune the scraper via `SCRAPER_CITY`, `SCRAPER_MAX_RENT`, `SCRAPER_INTERVAL_SECONDS`, `SCRAPER_REFRESH_HOURS`, `SCRAPER_MAX_AGE_DAYS`, `SCRAPER_MAX_PAGES`, `SCRAPER_KIND` (defaults in [`.env.example`](../.env.example)). Pagination is hard-capped at `SCRAPER_MAX_PAGES` (default 6) per `(source, kind)`; stubs whose posting date is older than `SCRAPER_MAX_AGE_DAYS` (default 4) are skipped without persisting and the loop continues. Set `SCRAPER_KIND=wg` (or `flat`) to restrict the scraper to one vertical; default `both`.
